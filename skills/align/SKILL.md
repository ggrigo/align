...
name: align
description: |
  Output review for LLM output. Generate an interactive HTML form where the user redlines each claim — correct, wrong, almost, needs-nuance, can't-verify, or skipped — plus add notes and flag missing items. Scales from a quick gut-check to a systematic eval pass. Submit downloads an align.md that Claude reads back to apply corrections to TASKS.md, smart-memory (guarded), and the per-session archive.
...
1. Walk its output and identify falsifiable claims. (For rhythm-shaped sources, the heuristics live in `references/claim-extraction-rhythm.md`. For other sources, the producer brings its own extraction logic.) **Cross-doc citations count.** When the source says *"per `X.md`"* or *"as documented in `Y`"*, that phrase is itself a falsifiable claim about `X`/`Y`'s content — verifiable by grep against the cited file. Extract these; don't skip them as mere attribution. **Location & visibility claims count too.** Phrases like *"public at `Z`"*, *"hosted at"*, *"lives in repo `R`"*, *"available at"* are falsifiable claims about *where something is and who can see it* — verifiable against actual repo/filesystem state (e.g. `gh api repos/owner/repo/contents/path`, or `ls`). These drift especially in self-describing artifacts (READMEs, essays, about-pages) where the author writes aspirationally — claiming something is public or at a path before it actually is. Extract and verify them rather than trusting the assertion.
